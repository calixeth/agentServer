...
Generate a {style} exaggerated character based on the provided image. wide shot, not close-up. Full-body portrait, facing the camera directly. Body and head both oriented forward, no tilt or side view. Highly detailed. The upper body is printed with sans-serif bold "KOLI" text.Width: 1024px, height: 1536px. Composition should be symmetrical and centered. Clear details preserved: original facial features, hairstyle, expression, body proportions, and clothing. Character occupies 70% of frame height, leaving 30% background visible. Subject should not dominate the image. Full head and hair entirely inside frame with ample headroom. Pure black background, no borders, no text.
"""
...
V_DEFAULT_PROMPT = """\
The character smiles gracefully, gently swaying from side to side with very small movements. After a moment, they rest one hand on their chin, maintaining a subtle, reserved smile, with a pure black background behind them. width: 1024px, height: 1536px, nagative:blur, distort, and low quality
"""

V_DANCE_IMAGE_PROMPT = """\
The frog dancing image is the base image. Generate the target image by replacing the first frog’s(in the front row) head and hands in the base image with the reference person’s portrait I provided. Medium shot, not close-up. The target image should be high-definition, highly detailed, seamlessly realistic, and with no sense of discord. At the same time, The character upper body is printed with sans-serif bold "KOLI" text. expand the generated image to the size: width 1024px, height 1536px.
"""
...
V_SING_IMAGE_PROMPT = """\
Replace the rock character’s head in Image 2 with the portrait from Image 1 to generate a high-definition 3D image. wide shot, not close-up. Leave 30% space between the top of the character’s head and the edge of the photo. The character’s face, hair, and expression should be rendered in high detail, with an exaggerated expression (referencing the expression in Image 2). The clothing should also be modified according to the character’s gender.The upper body is printed with sans-serif bold "KOLI" text. The final image size should be 1024px wide and 1536px high.Style: 3D cartoon / exaggerated caricature + mixed with realistic PBR materials.  Mood: Funny yet hardcore, with a heavy metal stage vibe. The character is designed as a comedic figure, but the lighting and materials pursue a “real stage” texture.  
"""

...
Generate a 3D cartoon Pixar-style exaggerated character based on the provided image. Medium shot, not close-up. Full-body portrait, facing the camera directly. Body and head both oriented forward, no tilt or side view. Both funny and romantic, creating a playful and humorous atmosphere. Highly detailed, clean sharp line art. Width: 1024px, height: 1536px. Composition should be symmetrical and centered. Clear details preserved: original facial features, hairstyle, expression, body proportions, and clothing. Character occupies 70% of frame height, leaving 30% background visible. Subject should not dominate the image. Full head and hair entirely inside frame with ample headroom. Pure black background, no borders, no text.
"""
...
V_SING_IMAGE_PROMPT = """\
Replace the rock character’s head in Image 2 with the portrait from Image 1 to generate a high-definition 3D image. Medium shot, not close-up. Leave 20% space between the top of the character’s head and the edge of the photo. The character’s face, hair, and expression should be rendered in high detail, with an exaggerated expression (referencing the expression in Image 2). The clothing should also be modified according to the character’s gender. The final image size should be 1024px wide and 1536px high.Style: 3D cartoon / exaggerated caricature + mixed with realistic PBR materials.  Mood: Funny yet hardcore, with a heavy metal stage vibe. The character is designed as a comedic figure, but the lighting and materials pursue a “real stage” texture. 
"""
